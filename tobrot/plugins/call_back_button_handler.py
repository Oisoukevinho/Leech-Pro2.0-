...
async def button(bot, update: CallbackQuery):
    ...
    if cb_data.startswith("gUPcancel"):
        ...
        if (int(update.from_user.id) == int(from_usr)) or g:
            ...
            await bot.answer_callback_query(
                ...
                text="Esta não é sua sanguessuga. Então, não toque nisso...😡😡",
                show_alert=True,
                cache_time=0,
            )
        return
    if "|" in cb_data:
        await bot.answer_callback_query(
            update.id, text="tentando baixar...", show_alert=False
        )
    ...
    if cb_data.startswith("cancel"):
        if (update.from_user.id == update.message.reply_to_message.from_user.id) or g:
            await bot.answer_callback_query(
                update.id, text="tentando cancelar...", show_alert=False
            )
            if len(cb_data) > 1:
                i_m_s_e_g = await update.message.reply_to_message.reply_text(
                    "verificando..?", quote=True
                )
                ...
                try:
                    ...
                    await i_m_s_e_g.edit_text(
                        f"Sanguessuga Cancelada por <a href='tg://user?id={update.from_user.id}'>{update.from_user.first_name}</a>"
                    )
                except Exception as e:
                    await i_m_s_e_g.edit_text("<i>FRACASSADA</i>\n\n" + str(e) + "\n#error")
        else:
            await bot.answer_callback_query(
                callback_query_id=update.id,
                text="Quem é Você? 🤪🤔🤔🤔",
    ...
    elif cb_data == "Foda se":
        if (update.from_user.id in AUTH_CHANNEL) or g:
            await bot.answer_callback_query(
                update.id, text="tentando excluir...", show_alert=False
            )
            g_d_list = [
                "app.json",
                "venv",
            ...
            if len(g_del_list) != 0:
                for f in g_del_list:
                    if os.path.isfile(f):
                        ...
                        shutil.rmtree(f)
                await update.message.edit_text(f"Objetos {len(g_del_list)} excluídos 🚮")
            else:
        ...
        await update.message.edit_text("Ok! multar 🤬")