...
async def upload_as_doc(client, message):
    ...
    await message.reply_text("**🗞 Seus arquivos serão carregados como documento 📁**")


async def upload_as_video(client, message):
    user_specific_config[message.from_user.id]=UserDynaConfig(message.from_user.id,False)
    await message.reply_text("**🗞 Seus arquivos serão enviados como Streamable 🎞**")
        ...
        for file in downloads:
            ...
            if file.status == "active":
                ...
                if is_file is None:
                    ...
                    msgg = f"<b>Pares:</b> {file.connections} | <b>Semeadores:</b> {file.num_seeders}"
...
async def cancel_message_f(client, message):
    if len(message.command) > 1:
        # /cancel command
        i_m_s_e_g = await message.reply_text("verificando..?", quote=True)
        aria_i_p = await aria_start()
        ...
        try:
            ...
            await i_m_s_e_g.edit_text("<i>FRACASSADO</i>\n\n" + str(e) + "\n#error")
    else:
        await message.delete()
...
async def upload_document_f(client, message):
    imsegd = await message.reply_text("em processamento ...")
    if message.from_user.id in AUTH_CHANNEL:
        if " " in message.text:
...
async def eval_message_f(client, message):
    if message.from_user.id in AUTH_CHANNEL:
        status_message = await message.reply_text("em processamento ...")
        cmd = message.text.split(" ", maxsplit=1)[1]
