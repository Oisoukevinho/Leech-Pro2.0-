...
async def incoming_message_f(client, message):
    ...
    if rep_mess:
        ...
        await i_m_sefg.edit("<b>Ei cara!</b>\n\n 🐈 <code>Responder com link direto/torrent</code>")
        return
    if dl_url is not None:
        ...
        await i_m_sefg.edit_text("Adicionado aos downloads. Mandar /status")
        ...
        await i_m_sefg.edit_text(
            f"**PORRA**! o que você digitou. \n<b>Erro de API</b>: {cf_name}"
        )
...
async def incoming_youtube_dl_f(client, message):
    ...
    i_m_sefg = await message.reply_text("<code>Em processamento...🔃</code>", quote=True)
    # LOGGER.info(message)
    ...
    if message.reply_to_message:
        ...
        await i_m_sefg.edit("<b>🐈 Ops, responda ao link compatível com YTDL.</b>")
        return
    if dl_url is not None:
        await i_m_sefg.edit_text("<b>Obtendo Formato Disponível</b>...")
        # create an unique directory
...
async def g_yt_playlist(client, message):
    ...
    if message.reply_to_message:
        ...
        await message.reply_text("<b> Responder com o link da lista de reprodução do Youtube</b>", quote=True)
        return
    if "youtube.com/playlist" in url:
        u_men = message.from_user.mention
        i_m_sefg = await message.reply_text(
            f"<b>Ok Tudo bem 🐈 {u_men} Mano!!:\n Sua solicitação foi ADICIONADA</b>\n\n <code> Aguarde até o Upload</code>",
            parse_mode="html",
        )
        await yt_playlist_downg(message, i_m_sefg, client, is_cloud)

    else:
        await message.reply_text("<b>Somente link da lista de reprodução do YouTube 🙄</b>", quote=True)
...
async def g_clonee(client, message):
    ...
    if message.reply_to_message is not None:
        ...
        await message.reply_text(
            "Você deve responder a uma mensagem, cujo formato deve ser [ID do arquivo/pasta Gdrive Nome do arquivo/pasta]\nOu leia o Github para obter informações detalhadas"
        )
...
async def rename_tg_file(client, message):
    ...
    if not message.reply_to_message:
        await message.reply("<b>Responder com Telegram Media</b> Nenhum", quote=True)
        return
    if len(message.command) > 1:
        ...
        try:
            ...
            if message_to_send != "":
                mention_req_user = (
                    f"<a href='tg://user?id={usr_id}'>🐈 Oi irmão!! Seus arquivos solicitados 👇</a>\n\n"
                )
                message_to_send = mention_req_user + message_to_send
                message_to_send = message_to_send + "\n\n" + "<b> #UPLOADS\n\n💫 Distribuído por : @Oisoukevinho</b>"
            else:
                message_to_send = "<i>FRACASSADO</i> para fazer upload de arquivos. 😞😞"
            await message.reply_text(
        ...
        await message.reply_text(
            "<b> Ops 😬</b>\n\nForneça Nome com extensão\n\n➩<b>Exemplo</b>: <code> /rename Avengers Endgame.mkv</code>", quote=True
        )
