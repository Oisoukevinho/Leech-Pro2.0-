    ...
    if os.path.exists(torrent_file_path):
        ...
        try:
            ...
            return (
                ...
                "**FRACASSADO 😭** \n"
                + str(e)
                + " \n<b> Seu link está lento cara 😅🐈</b>",
            )
        else:
            return True, "" + download.gid + ""
    else:
        return False, "**FRACASSADO 😭** \nPor favor, tente outras fontes para obter um link viável"
...
def add_url(aria_instance, text_url, c_file_name):
    ...
    try:
        ...
        return (
            False,
            "**FRACASSADO 😭** \n" +
            str(e) + " \Por favor, não envie links LENTOS 😤😤🤬. Read /help",
        )
    else:
        return True, "" + download.gid + ""
        ...
        try:
            ...
            if not complete:
                if not file.error_message:
                    if file.has_failed:
                        LOGGER.info(
                            f"O cancelamento do download de {file.name} pode ser devido a um torrent lento"
                        )
                        await event.reply(
                ...
                LOGGER.info(
                    f"Baixado com sucesso 🙏: `{file.name} ({file.total_length_string()})` 🤒"
                )
                # await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
