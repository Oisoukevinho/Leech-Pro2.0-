    ...
    if os.path.isdir(local_file_name):
        ...
        if os.path.getsize(local_file_name) > TG_MAX_FILE_SIZE:
            ...
            i_m_s_g = await message.reply_text(
                "O Telegram não suporta o upload deste arquivo.\n"
                f"Tamanho do arquivo detectado: {d_f_s} 😡\n"
            ...
            await i_m_s_g.edit_text(
                ...
                "Tentando enviar para o Telegram, agora... 🙏🙏"
            )
...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    del_it = await message.edit_text(
        f"<a href='tg://user?id={g_id}'>🔊</a> Agora carregando para ☁️ Cloud!!!"
    )
    ...
    if os.path.isfile(file_upload):
        ...
        await messa_ge.reply_text(
            f"🤖: Enviado com sucesso `{os.path.basename(file_upload)}` <a href='tg://user?id={g_id}'>🤒</a>\n📀 Size: {gjay}",
...
async def upload_single_file(
    ...
    # todo
        ...
        sent_message = await message.reply_document(
            ...
            progress_args=(
                f"**• Enviando :** `{os.path.basename(local_file_name)}`",
                start_time,
            ),
        )
